...
assert(#list == 2, "two tabs expected, got " .. #list)
assert(list[1].tab == 1 and list[2].tab == 3, "tabs must be ordered by tab index")
assert(list[1].name == "One" and list[1].icon == 111, "tab name/icon must come from the cache")
assert(list[1].withdrawals == -1 and list[2].withdrawals == 25,
    "remainingWithdrawals must be carried (tooltip data)")
assert(not list[1].purchase and not list[2].purchase, "real tabs carry no purchase marker")

-- Test 3: single mid-range tab survives alone
list = GuildWindow.BuildTabList({ tabs = { [2] = { size = 98, slots = {}, name = "Solo" } } }, nil)
assert(#list == 1 and list[1].tab == 2 and list[1].name == "Solo",
    "a lone sparse tab must list by itself")

-- Test 4: liveViewable filtering — false drops the tab; absent and true keep
list = GuildWindow.BuildTabList(rec, { liveViewable = { [1] = false } })
assert(#list == 1 and list[1].tab == 3,
    "liveViewable[tab]=false must drop that tab")
list = GuildWindow.BuildTabList(rec, { liveViewable = { [1] = true } })
assert(#list == 2, "liveViewable[tab]=true must keep the tab")
list = GuildWindow.BuildTabList(rec, { liveViewable = {} })
assert(#list == 2, "absent liveViewable keys must default to viewable")
list = GuildWindow.BuildTabList(rec, { liveViewable = { [1] = false, [3] = false } })
assert(#list == 0, "filtering every tab must yield an empty list")

-- Test 5: canPurchase appends the marker AFTER the real tabs
list = GuildWindow.BuildTabList(rec, { canPurchase = true })
assert(#list == 3, "purchase marker expected, got " .. #list)
assert(list[3].purchase == true, "marker must come last")
assert(list[3].tab == nil, "purchase markers carry no tab index")
assert(list[1].tab == 1 and list[2].tab == 3, "real tabs must precede the marker")
...
-- Test 7: no opts table → no purchase marker (cached mode)
...
assert(#list == 2, "empty opts must add no marker")
for _, entry in ipairs(list) do
