...
local function Store()
    ...
    if store.groupLayoutVersion == nil then
        for name, group in pairs(store.groups) do
            if type(group) ~= "table" then
                group = {}
                store.groups[name] = group
            end
            group.layoutEnabled = false
        end
        for _, display in pairs(store.displays) do
            local name = type(display) == "table" and display.group
            if type(name) == "string" and name ~= "" and store.groups[name] == nil then
                store.groups[name] = { layoutEnabled = false }
            end
        end
        store.groupLayoutVersion = GROUP_LAYOUT_VERSION
    end
    return store
end
AD.Store = Store
...
function AD.GroupUsesLayout(groupName)
    local group = AD.GetGroup(groupName, false)
    return group ~= nil and group.layoutEnabled ~= false
end
...
function AD.SetGroupParent(groupName, parentName)
    ...
    local parentGroup = EnsureGroup(store, parent)
    if GroupDepth(store, parent) >= MAX_GROUP_DEPTH then
        return false, "depth"
    end
    group.layoutEnabled = true
    parentGroup.layoutEnabled = true
    group.parent = parent
    return true
end
...
function AD.CommitPreviewDragPosition()
    ...
    hx, hy = ns.Helpers.SafeNumberOrNil(hx), ns.Helpers.SafeNumberOrNil(hy)
    ux, uy = ns.Helpers.SafeNumberOrNil(ux), ns.Helpers.SafeNumberOrNil(uy)
    hs, us = ns.Helpers.SafeNumberOrNil(hs) or 1, ns.Helpers.SafeNumberOrNil(us) or 1
...
ApplyDisplay = function(display, allowCreate)
    ...
    local grouped = AD.GroupUsesLayout(display.group)
...
function AD.ReflowGroups(displays)
    ...
    for i = 1, #displays do
        ...
        if key and AD.GroupUsesLayout(key) then
...
function AD.RegisterLayoutElement(display)
    if AD.GroupUsesLayout(display.group) then return end
...
function AD.Refresh()
    ...
    if store and store.enabled ~= false then
        ...
        for i = 1, #displays do
            local display = displays[i]
            local groupName = GroupKey(display.group)
            if groupName and AD.GroupUsesLayout(groupName) then
