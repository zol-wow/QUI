...
QUI_HelpContent.Diagnostics = {
    ...
    -- Combat-end profiler
    { label = "Combat Profiler On",   command = "/qui combatprof on",
      tooltip = "Start profiling the PLAYER_REGEN_ENABLED handler chain. Wraps named CDM functions plus every frame in QUI_PerfRegistry (CDM, group frames, action bars, raid buffs, aura dispatch, plus skinning combat-defer frames) and prints a summary 2.5s after each combat ends. Use to diagnose combat-end stutter.",
      run = function() QUI:SlashCommandOpen("combatprof on") end },
    { label = "Combat Profiler Off",  command = "/qui combatprof off",
      tooltip = "Stop the combat-end profiler and unwrap functions.",
      run = function() QUI:SlashCommandOpen("combatprof off") end },
    { label = "Combat Profiler Report", command = "/qui combatprof report",
      tooltip = "Reprint the most recent combat-end report.",
      run = function() QUI:SlashCommandOpen("combatprof report") end },
    { label = "Combat Profiler Reset", command = "/qui combatprof reset",
      tooltip = "Clear the combat-end profiler's accumulated stats.",
