---------------------------------------------------------------------------
-- Bags data layer: player bag scanner (bag IDs 0–5: backpack, bags 1–4,
...
function ScanBags.Drain()
    ...
    for bagID in pairs(toScan) do
        rec.bags[bagID] = Bags.ScanCommon.ReadContainer(bagID, Bags.ScanCommon.MakePendingHandler(bagID, ScanBags.MarkDirty))
        changed[#changed + 1] = bagID
    end
    if #changed > 0 then
        -- changed: unordered array of bagIDs (consumers must not assume order)
        Bags.Bus.Publish("BagsChanged", Bags.Store.GetCurrentCharacterKey(), changed)
        return true
    end
    return false
end
